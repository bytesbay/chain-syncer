...
// EXPORTS
__webpack_require__.d(__webpack_exports__, "ChainSyncer", function() { return /* reexport */ chain_syncer_ChainSyncer; });
__webpack_require__.d(__webpack_exports__, "InMemoryAdapter", function() { return /* reexport */ in_memory_adapter_InMemoryAdapter; });
...
const addEvents = async function (scans) {
  ...
  const process_events = scans.map(item => item.events.map(event => {
    return this.parseEvent(item.contract_name, event, used_blocks.find(n => n.number === event.blockNumber), used_txs.find(n => n.hash === event.transactionHash));
  })).reduce((acc, n) => {
    return [...acc, ...n];
  }, []);
...
const addListener = async function (event, listener) {
  if (this.mode !== 'mono') {
...
const updateSubscriber = async function (subscriber, events) {
  const {
    events_added,
    events_removed
  } = await this.adapter.updateSubscriber(subscriber, events);
...
const getContractEvents = async function (contract_name, max_block, opts = {}) {
  ...
  if (!from_block) {
    ...
    try {
      var transaction = await this.ethers_provider.getTransaction(contract.deployed_transaction_hash);
    } catch (error) {
...
const helpers = {
  ...
  async _loadUsedBlocks(events) {
    ...
    used_blocks = await Promise.all(used_blocks.map(n => this.ethers_provider.getBlock(n).catch(err => {
  ...
  async _loadUsedTxs(events) {
    ...
    used_txs = await Promise.all(used_txs.map(n => this.ethers_provider.getTransaction(n).catch(err => {
...
// CONCATENATED MODULE: ./src/lib/chain-syncer/parse-event.js
...
const processSubscriberEvents = async function (subscriber) {
  // get all unprocessed events by contract and event name
  let events = await this.adapter.selectAllUnprocessedEventsBySubscriber(subscriber);
...
const safeRescan = async function (max_block) {
  ...
  const {
    scans,
    events
  } = await this.scanContracts(max_block, {
    force_rescan_till
  });
...
const saveLatestBlocks = async function (scans) {
  await Promise.all(scans.map(n => this.adapter.saveLatestScannedBlockNumber(n.contract_name, n.block)));
};
...
const scanContractBlocks = async function (contract, contract_name, from_block, to_block, max_block) {
  ...
  const event_ids = await this.adapter.filterExistingEvents(events.map(n => this._parseEventId(n)));
  events = events.filter(n => {
...
const scanContracts = async function (max_block, opts = {}) {
  let proms = [];
  ...
  for (const i in _contracts) {
    ...
    const prom = this.getContractEvents(contract_name, max_block, opts).catch(err => {
...
const syncSubscribers = async function () {
...
// CONCATENATED MODULE: ./src/lib/chain-syncer/index.js
...
class chain_syncer_ChainSyncer {
  constructor(adapter, opts = {}) {
    ...
    _defineProperty(this, "updateSubscriber", updateSubscriber);
    ...
    _defineProperty(this, "parseEvent", parseEvent);
  ...
  async markEventsAsProcessed(subscriber, event_ids) {
    await Promise.all(event_ids.map(n => this.adapter.setEventProcessedForSubscriber(n, subscriber)));
  }

}
// CONCATENATED MODULE: ./src/lib/in-memory-adapter/Event.js
...
class in_memory_adapter_InMemoryAdapter {
  constructor() {
  ...
  async addUnprocessedEventsToQueue(subscriber, events) {
    events.forEach(e => {
      ...
      this.events_queue.push(...unprocessed_events.map(n => new QueueEvent_QueueEvent(n, subscriber)));
    });
  }
  /**
   * 
  ...
  async saveEvents(events, subscribers) {
    ...
    events = events.map(n => new Event_Event(n));
    ...
    for (const i in subscribers) {
      ...
      this.events_queue.push(...filtered_events.map(n => new QueueEvent_QueueEvent(n, subs.name)));
    }
  }
...
/* harmony default export */ var lib = (chain_syncer_ChainSyncer);
...
//# sourceMappingURL=chain-syncer.umd.js.map